...
describe('Body in Table', () => {
  it('Body will render empty body when dataset is empty', () => {
    const wrapper = mount(<Table columns={columns} datasets={[]} rowKey='item_id' autoStick />);
  ...
  it('Table can have custom getRowConf prop, and will change acting of Body', () => {
    // HACK: branch
    const getRowConfMock = jest.fn().mockImplementation(() => { return {} });
    let wrapper = mount(<Table columns={columns} datasets={datasets} rowKey='foo_id' getRowConf={getRowConfMock} selection={{ selectedRowKeys: [] }} />);
    const rowConfMock = jest.fn().mockImplementation(() => { return { rowClass: 'prefix', canSelect: false } });
    const onSelectMock = jest.fn();
    wrapper = mount(<Table columns={columns} datasets={datasets} rowKey='foo_id' getRowConf={rowConfMock} selection={{ selectedRowKeys: [], onSelect: onSelectMock }} />);
    ...
    wrapper = mount(<Table columns={columns} datasets={datasets} rowKey='item_id' getRowConf={getRowConfMock} selection={{ selectedRowKeys: [], onSelect: onSelectMock }} />);
  ...
  describe('Td in Table', () => {
    let wrapper = mount(<Table columns={columns} datasets={datasets} rowKey='item_id' />);
