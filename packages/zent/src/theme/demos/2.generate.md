...
```jsx
import { ColorPicker, Notify, CopyButton, Button, Select, ThemeCssVars } from 'zent';
...
const defaultTheme = generateTheme({ colors: [{ baseColor: '#155bd4', scene: colorScene }] }, ThemeCssVars);
...
const ThemeSdkComponent = () => {
  ...
  const changeTheme = (baseColor, scene) => {
    const theme = generateTheme({ colors: [{ baseColor, scene }] }, ThemeCssVars);
