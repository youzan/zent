...
export default class SideNav extends Component {
  ...
  parseData = (item, index) => (
    <li className="nav-item" key={`nav-${index}`}>
      {item.path
        ? <NavLink activeClassName="active" exact to={this.props.base + item.path}>{item.name}</NavLink>
        : <a href="javascript:void(0)" onClick={(e) => this.handleTitleClick(item)}>{item.name}</a>}
      {item.children && <ul className="pure-menu-list sub-nav">
  ...
  parseChildren = (navItem, index) => (
    <li className="nav-item" key={`nav-children-${index}`}>
      <NavLink activeClassName="active" exact to={this.props.base + navItem.path}>{navItem.title || navItem.name}</NavLink>
    </li>
  )
  ...
  parseList = (navItem, index) => {
    return navItem.disabled
      ...
      : <li className="nav-item" key={`nav-list-${index}`}>
        <NavLink activeClassName="active" exact to={this.props.base + navItem.path}>{navItem.title}</NavLink>
      </li>
  }
